...
export default function IncomeAnalyticsPage() {
  ...
  const incomes = sourceFiltered.filter((tx) => tx.type === 'income');
  ...
  incomes.forEach((tx) => { catMap.set(tx.category, (catMap.get(tx.category) || 0) + tx.amount); });
  const pieData = Array.from(catMap.entries()).map(([name, value]) => ({ name, value }));

  const hourMap = new Map<number, number>();
  incomes.forEach((tx) => { const hour = new Date(tx.timestamp).getHours(); hourMap.set(hour, (hourMap.get(hour) || 0) + tx.amount); });
  ...
  for (let i = heatmapDays - 1; i >= 0; i--) {
    ...
    const dayIncome = incomes.filter((tx) => tx.timestamp >= dayStart && tx.timestamp < dayEnd).reduce((sum, tx) => sum + tx.amount, 0);
    heatmapData.push({ date: dateStr, amount: dayIncome, day: i });
  }
  ...
  const getCategoryTransactions = (categoryName: string) => {
    return incomes.filter((tx) => tx.category === categoryName);
  };
    ...
    <div className="px-4 pt-4" style={{ paddingBottom: '2cm' }}>
      <StaggeredEntrance index={0}>
        <div className="flex items-center justify-between mb-6">
          <motion.h1 className="text-2xl font-bold" style={{ color: colors.text }} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>Analitok</motion.h1>
          <motion.div
        ...
        <div className="flex gap-2 mb-4">
          {(['semua', 'online', 'offline'] as const).map((f, i) => (
            <motion.button
              ...
              style={sourceFilter === f ? { background: `${colors.offline}33`, color: colors.offline, borderColor: `${colors.offline}66` } : { color: colors.textMuted, borderColor: colors.borderSubtle }}
      ...
      <StaggeredEntrance index={2} variant="scaleIn">
        <motion.div
          ...
          transition={{ type: 'spring', stiffness: 400, damping: 30 }}
        >
          <p className="text-xs mb-1" style={{ color: colors.textMuted }}>Total Pemasukan{sourceFilter !== 'semua' ? ` (${sourceFilter})` : ''}</p>
          <motion.p
          ...
          <p className="text-xs mt-1" style={{ color: colors.textMuted }}>{incomes.length} transaksi</p>
        </motion.div>
      </StaggeredEntrance>
      ...
      <StaggeredEntrance index={3} variant="flipUp">
        <motion.div
          ...
          whileHover={{ y: -1 }}
        >
          <h3 className="text-sm font-semibold mb-3" style={{ color: colors.text }}>Pemasukan Harian</h3>
          {burnData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={burnData}>
                ...
                <Tooltip contentStyle={tooltipStyle} labelStyle={{ color: colors.textMuted }} formatter={(value) => [formatRupiah(Number(value)), 'Pemasukan']} />
                <Line type="monotone" dataKey="amount" stroke={colors.offline} strokeWidth={2.5} dot={false} animationDuration={1500} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </motion.div>
      ...
      <StaggeredEntrance index={4} variant="scaleIn">
        <motion.div className="rounded-3xl p-4 border mb-4" style={{ background: colors.card, borderColor: colors.border }} whileHover={{ y: -1 }}>
          <h3 className="text-sm font-semibold mb-3" style={{ color: colors.text }}>Kategori Pemasukan</h3>
          {pieData.length === 0 ? (
            <motion.p className="text-xs text-center py-8" style={{ color: colors.textMuted }} animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 3, repeat: Infinity }}>Belum ada data</motion.p>
          ) : (
      ...
      <StaggeredEntrance index={5} variant="slideRight">
        <motion.div className="rounded-3xl p-4 border mb-4" style={{ background: colors.card, borderColor: colors.border }} whileHover={{ y: -1 }}>
          <h3 className="text-sm font-semibold mb-3" style={{ color: colors.text }}>Waktu Penerimaan</h3>
          <ResponsiveContainer width="100%" height={120}>
            <LineChart data={timeData}>
              ...
              <Tooltip contentStyle={tooltipStyle} formatter={(value) => [formatRupiah(Number(value)), 'Pemasukan']} />
      ...
      <StaggeredEntrance index={6} variant="fadeUp">
        <motion.div className="rounded-3xl p-4 border" style={{ background: colors.card, borderColor: colors.border }} whileHover={{ y: -1 }}>
          <h3 className="text-sm font-semibold mb-3" style={{ color: colors.text }}>Heatmap Pemasukan (90 Hari)</h3>
            ...
            {heatmapData.map((d, i) => {
              ...
              return (
                <motion.div
                  ...
                  style={{ background: `${colors.offline}${Math.round(intensity * 255).toString(16).padStart(2, '0')}` }}
